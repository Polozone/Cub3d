...
/*   Updated: 2022/11/15 13:54:07 by tdeville         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	render_walls(t_data *data, int drawStart, int drawEnd, int x, int color, char *name, int mapX)
{
	...
	while (drawStart < drawEnd)
	{
		...
		color2 = get_color_from_xpm(data, x, (drawEnd - drawStart) * ratio);
...
void	render_line(t_data *data, int color, int drawStart, int drawEnd, int x, char *name, int mapX)
{
	...
	render_walls(data, drawStart, drawEnd, x, color, name, mapX);
...
void	render_wall(t_data *data, double sideDistX, double sideDistY, int side, double deltaDistX, double deltaDistY, int mapX, int mapY, int x, int stepX, int stepY)
{
	...
	if (side == 0) // E/W
	{
		if (stepX == 1)
		{
			render_line(data, 0xFFFF00, drawStart, drawEnd, x, "path_to_the_west_texture.xpm", mapX);
			return ;
		}
		else if (stepX == -1)
		{
			render_line(data, 0xFFFFFF, drawStart, drawEnd, x, "path_to_the_east_texture.xpm", mapX);
			return ;
		}
	}
	else // N/S
	{
		if (stepY == 1)
		{
			render_line(data, 0x000000, drawStart, drawEnd, x, "path_to_the_north_texture.xpm", mapX);
			return ;
		}
		else if (stepY == -1)
		{
			render_line(data, 0x009911, drawStart, drawEnd, x, "path_to_the_south_texture.xpm", mapX);
			return ;
		}
	}
	return ;
